...
from flask import Blueprint, render_template, jsonify, request, json, url_for, Flask
...
#app.config['MAIL_USERNAME'] = 'seminarspace10@gmail.com'
#app.config['MAIL_PASSWORD'] = 'cqlq qwcb eeeo pytv'
#app.config['MAIL_DEBUG'] = True
#app.config['MAIL_SUPPRESS_SEND'] = False

# Initialize Flask-Mail

#mail.init_app(app)  # Add this line after creating the Mail instance
...
def register():
    ...
    input_array3 = json.loads(request.args.get('inputArray3'))

    print(listToString(input_array1))
    print(listToString(input_array2))
    print(listToString(input_array3))
    # Insert file names and encodings into MongoDB collection
    insert_file_names_and_encodings(known_dir, filtered_file_names, input_array1, input_array2, input_array3)
    ...
    print(f"Input Array 3: {input_array3}")

    # Return the file names and message
    # Return the file names, input arrays, and message
    return jsonify(
        {"file_names": file_names, "input_array1": input_array1, "input_array2": input_array2, "input_array3": input_array3, "message": message})


def insert_file_names_and_encodings(known_dir, file_names,input_array1, input_array2,input_array3):
    # Insert file names and encodings into MongoDB collection
    client = MongoClient("mongodb://localhost:27017")
    db = client['ADT_project']
    collection = db['face_encodings']
    ...
    for file in os.listdir(unknown_dir):
        ...
        try:
            ...
            if selected_workshop and (selected_workshop not in user_data.get("workshop_list", [])):
                print("addeddddddd");
                print(user_data.get("workshop_list", []))
...
def register_workshop():
    try:
        ...
        emails, ids = get_all_emails()

        # Send email to each email address
        for email, idss in zip(emails, ids):
            send_email(email, idss, workshop_name, presenter_name)
...
def get_all_emails():
    # Retrieve emails from the "face_encodings" collection
    client = MongoClient("mongodb://localhost:27017")
    db = client['ADT_project']
    collection = db['face_encodings']
    data = list(collection.find({}, {'_id': 0, 'Email': 1, 'ID': 1}))

    emails = [item['Email'] for item in data if 'Email' in item]
    ids = [item['ID'] for item in data if 'ID' in item]
    return emails, ids

def send_email(email,idss, workshop_name, presenter_name):
    try:
        #unique_identifier = generate_unique_identifier()

        # Update the user's email status link
        update_link = f"{request.url_root}update_email_status/{email}/{workshop_name}/{idss}"
        subject = f"Workshop Registration: {workshop_name}"
        body = f"Dear participant,\n\nThank you for registering in seminar space. New workshop: '{workshop_name}' by {presenter_name}.Click the following link to register: '{update_link}' "
        email_sender = 'seminarspace10@gmail.com'
        email_pass = 'kdwq mkio xmvz dpsl'
...
def update_workshop_list():
    try:
        ...
        if workshop in student_data.get("workshop_list", []):
            print(student_data.get("workshop_list", []))
...
@task_controller.route('/update_email_status/<unique_identifier>/<workshop_name>/<idss>')
def update_email_status(unique_identifier, workshop_name,idss):
    try:
        # Retrieve the user based on the unique identifier
        client = MongoClient("mongodb://localhost:27017")
        db = client['ADT_project']
        collection = db['workshop_dets']

        query = {"workshop_name": workshop_name}
        user_data = collection.find_one(query)

        # Add workshop_name to reg_works
        if user_data:
            collection.update_one(query, {"$addToSet": {"reg_works": idss}})

            message = f"Workshop {workshop_name} added to reg_works for user: {idss}."
            return jsonify({"message": message})
        else:
        ...
        error_message = f"Error updating email status: {str(e)}"
        return jsonify({"error": error_message})


@task_controller.route('/get_workshops')
def get_workshops():
    try:
        # Retrieve workshop names from MongoDB collection
        client = MongoClient("mongodb://localhost:27017")
        ...
        error_message = f"Error fetching workshops: {str(e)}"
        return jsonify({"error": error_message})


@task_controller.route('/get_registrations')
def get_registrations():
    try:
        # Get the selected workshop name from the request
        workshop_name = request.args.get('workshopName')

        # Retrieve registrations for the selected workshop from MongoDB collection
        client = MongoClient("mongodb://localhost:27017")
        db = client['ADT_project']
        collection = db['workshop_dets']

        query = {"workshop_name": workshop_name}
        workshop_data = collection.find_one(query)

        if workshop_data:

            registrations = workshop_data.get("reg_works", [])
            return jsonify({"registrations": registrations})
        else:
            error_message = f"No data found for workshop: {workshop_name}"
            return jsonify({"error": error_message})
    except Exception as e:
        error_message = f"Error fetching registrations: {str(e)}"
        return jsonify({"error": error_message})